...
<%
  ...
  String SQL = "select cb.id_cuenta, cb.num_cuenta ||'     '|| cc.nombre_cta nombre_cta, cc.cuenta_cheques_id " + 
     "from rf_tesoreria.rf_tr_cuentas_bancarias cb, rf_tc_cuentas_cheques cc " +
     "where cb.id_cuenta = cc.id_cuenta and cc.unidad_ejecutora = "+ lsUnidad +" and cc.ambito = "+lsAmbito+" and cc.entidad = "+lsEntidad +
     "and cc.cuenta_cheques_id = (SELECT CUENTA_CHEQUES_ID FROM rf_tc_cuentas_cheques WHERE TIPO_FORMATO = 'U' AND "+ lsEjercicio +
...
function revisaFormulario() 
    {
           ...
           if ((document.getElementById('txtFecha01').value=="") && (!document.getElementById('txtFecha02').value==""))
           {
               valida01=false;
            ...
            if (valida01)
            {
             if (!(document.getElementById('txtFecha01').value=="" && document.getElementById('txtFecha02').value=="")) 
             {      
               if (!(document.getElementById('txtFecha01').value=="")) 
               {
                   valida02=ValidarObjeto(document.getElementById('txtFecha01').value,"F");
                   if (!(valida02)) {
                       cadenaError=cadenaError+"Error en el formato de la fecha, verifique que sea correcto(dd/mm/aaaa): "+document.getElementById('txtFecha01').value+"\n";
                       valida01=false; 
                   }
                   else date01=convierte(document.getElementById('txtFecha01').value);
               }//txtFecha01


               if (!(document.getElementById('txtFecha02').value=="")) 
               {
                   valida02=ValidarObjeto(document.getElementById('txtFecha02').value,"F");
                   if (!(valida02)) 
                   {                         
                    cadenaError=cadenaError+"Error en el formato de la fecha, verifique que sea correcto(dd/mm/aaaa): "+document.getElementById('txtFecha02').value+"\n";
                    valida01=false; 
                   }
                   else date02=convierte(document.getElementById('txtFecha02').value);            
                }//txtFecha02
  ...
  <body>
    ...
    <FORM name="filtro" method="post"  action="c705Resultado.jsp">
...
<%      
  ...
  <table cellpadding="5"  border="0" width="15%" class="sianoborder" align="center">
    <tr align="center">
      <td  align="right"><input type='button' name='btnAceptar' value='Aceptar' class='boton' onClick="javascript:if(revisaFormulario()){}"></td>
      <td><input type='button' name='btnRegresar' value='Regresar' class='boton' onClick="javascript:LlamaPagina('filtroGeneral.jsp?opcion=aplicaCheques&idCatalogoCuenta=1','');"></td>
    </tr>
  </table>
  <br>
    </FORM>
