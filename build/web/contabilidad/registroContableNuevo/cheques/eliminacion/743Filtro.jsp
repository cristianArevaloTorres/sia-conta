...
<%
  ...
  String SQL = "select cb.id_cuenta, cb.num_cuenta ||'     '|| cb.nombre_cta nombre_cta " + 
     ...
     "where cb.id_cuenta = cc.id_cuenta and cc.unidad_ejecutora = "+ lsUnidad +" and cc.ambito = "+lsAmbito+" and cc.entidad = "+lsEntidad+"  and cb.id_tipo_cta in(1,2)" +
...
function revisaFormulario() {
  ...
  <body>
      <jsp:scriptlet>util.tituloPagina(out, "Contabilidad", " ", "Filtro de eliminacion de carga de pólizas de cheques", "Eliminar", true);</jsp:scriptlet>    
        
    <H2>Informaci&oacute;n: Unidad ejecutora=<%=lsUnidad%>, Entidad=<%=lsEntidad%>, Ambito=<%=lsAmbito%> y Ejercicio=<%=lsEjercicio%> </H2>
    
    <br>
    
    <FORM name="filtro" method="post"  action="../eliminacion/743Resultado.jsp">
...
<%      
      ...
      <tr>
          ...
          <td><input type='button' name='btnRegresar' value='Regresar' class='boton' onClick="window.open('../../../registroContableNuevo/filtroGeneral.jsp?opcion=eliminarCarga&idCatalogoCuenta=1','_self');"></td>
      </tr>
    </table>            
    </FORM>
  </body>
