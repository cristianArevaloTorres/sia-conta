...
<html>
  <head>
    ...
    <title>tesComCarteraIFFCtrl</title>
    ...
    private String getQueryReporte(HttpServletRequest request, String idCuentaS){
        ...
        try  {      
            ...
            regresa = sentencia.getCommand("consultas.select.compCarteraFechaInversion.reportesInversiones");
        } catch (Exception e)  {
           e.printStackTrace();
        }  finally  {
              sentencia = null;
...
//        bancoCuenta = getBancoCuenta(request);
 ...
 //       parametros.put("CUENTAINVERSION",bancoCuenta.substring(bancoCuenta.indexOf(",")+1,bancoCuenta.length()));
 
        datosCtaInv = getBancoCuenta(request).split(",");
        parametros.put("INSTBANCARIA",datosCtaInv[0]);
        parametros.put("CUENTAINVERSION",datosCtaInv[1]);
        ...
        if (request.getParameter("tipoReporte").equals("pdf"))
            request.getSession().setAttribute("rutaArchivo","/tesoreria/reportesInversion/Reportes/comCarteraIFF");
        else
            request.getSession().setAttribute("rutaArchivo","/tesoreria/reportesInversion/Reportes/comCarteraIFFXls");
