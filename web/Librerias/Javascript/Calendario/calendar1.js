...
function cal_popup1 (str_datetime) {
	...
	var obj_calwindow = window.open(
		'../Librerias/Javascript/Calendario/calendar.html?datetime=' + this.dt_current.valueOf()+ '&id=' + this.id,
...
function cal_prs_time1 (str_time, dt_date) {
	...
	if (!arr_time[0]) dt_date.setHours(0);
		...
		else return cal_error ("Hora inválida: '" + arr_time[0] + "'.\nRango permitido es 00-23.");
	else return cal_error ("Valor de hora inválida: '" + arr_time[0] + "'.\nAllowed values are unsigned integers.");
	
	if (!arr_time[1]) dt_date.setMinutes(0);
	else if (RE_NUM.exec(arr_time[1]))
		if (arr_time[1] < 60) dt_date.setMinutes(arr_time[1]);
		else return cal_error ("Minuto inválido: '" + arr_time[1] + "'.\nRango permitido es 00-59.");
	else return cal_error ("Valor de minuto inválido: '" + arr_time[1] + "'.\nAllowed values are unsigned integers.");
