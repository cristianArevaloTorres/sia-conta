...
<html>
    ...
    <body >
        ...
        <br>
        <FORM name="resultado" id="resultado" Method="post" action="c748Control.jsp" onsubmit='return revisarForm();' >
                        ...
                        <%
                            ...
                            try {
                                ...
                                Query = "select ROWNUM CONSFILA, sqlresul.* from ("
                                        + "SELECT distinct a.poliza_id polizaid,a.poliza_id||'-'||cheque.cheque_id ids,cheque.consecutivo as chequeCons, b.abreviatura||a.consecutivo as consecutivo,to_char(a.fecha,'dd/mm/yyyy') fecha,cheque.beneficiario, a.concepto,a.origen, 'Cheque' as poliza "
                                        + "FROM RF_TR_POLIZAS a,  RF_TC_TIPOS_POLIZAS b, RF_TR_DETALLE_POLIZA d, RF_TR_CUENTAS_CONTABLES c,RF_TR_CHEQUES cheque, RF_TC_CUENTAS_CHEQUES CC "
                                        + "where a.poliza_id=d.poliza_id  and a.ban_cheque=1 "
                                        + "and a.poliza_id=cheque.poliza_id and d.cuenta_contable_id=c.cuenta_contable_id and a.tipo_poliza_id=b.tipo_poliza_id and "
                                        + "a.id_catalogo_cuenta=" + controlRegistro.getIdCatalogoCuenta() + " and a.unidad_ejecutora='" + controlRegistro.getUnidad()
                                        + "' and a.ambito=" + controlRegistro.getAmbito() + " and a.entidad=" + controlRegistro.getEntidad() + lsCondicion + lsAnio
                                        ...
                                        + "UNION "
                                        + "SELECT a.poliza_id polizaid,a.poliza_id||'-'||cheque.cheque_id ids,cheque.consecutivo as chequeCons, b.abreviatura||a.consecutivo as consecutivo,"
                                        + "to_char(a.fecha,'dd/mm/yyyy') fecha,cheque.beneficiario, a.concepto,a.origen, 'Cheque' as poliza "
                                        + "from (select h.*, ch.id_cuenta from rf_tr_cheques h, rf_tc_cuentas_cheques ch  where ch.cuenta_cheques_id = h.cuenta_cheques_id) cheque, "
                                        ...
                                        + "from rf_tesoreria_ift.rf_tr_movimientos_cuenta mm where mm.fecha_hora > to_date('2011/12/30','yyyy/mm/(dd') and mm.id_clave_trans in (291,308) ) m, "
                                        + "rf_tesoreria_ift.rf_tr_cuentas_bancarias c, rf_tesoreria_ift.rf_tc_estatus_cheque ec, rf_tr_polizas a, rf_tc_tipos_polizas b "
                                        + "where m.id_cuenta(+) = cheque.id_cuenta and cheque.consecutivo =  m.numcheque(+) and c.id_cuenta(+) = cheque.id_cuenta and a.poliza_id = cheque.poliza_id "
                                        + "and a.tipo_poliza_id=b.tipo_poliza_id and ec.id_estatus_cheque = decode(m.id_movimiento,null,decode(cheque.estatus, 3, 3, 5, 4, 2),1) "
                                        + "and c.id_cuenta=" + lsCuentaBancaria + " and a.unidad_ejecutora='" + controlRegistro.getUnidad() + "' and a.ambito=" + controlRegistro.getAmbito() + "  and a.entidad=" + controlRegistro.getEntidad() + lsCondicion
                                        + "and  decode(m.id_movimiento, null, decode(cheque.estatus, 3, 3, 5, 4, 2), 1) in (2) and (trunc(cheque.fechacheque) >= to_date('01/03/2011','dd/mm/yyyy'))"
                                        + "order by polizaid asc) sqlresul ";

                                System.out.println("Query c748Resultado: " + Query);      
                                String[] DefQuery = {Query, "100%", "LEFT", "2", ""};
                                String[] DefAlias = {"CHEQUECONS-,CONSECUTIVO-,FECHA-,BENEFICIARIO-,CONCEPTO-,", "9%,9%,9%,30%,30%", "Consecutivo Cheque,Consecutivo Poliza,Fecha,Beneficiario,Concepto", "4,4,4,4,4"};
            ...
            <table width='100%'>
                <tr><td width='73%'>&nbsp;</td>
                    ...
                    <td width='80%'>
                        <input type='button' name='btnRegresar' value='Regresar' class='boton' onClick="javascript:LlamaPagina('c748Filtro.jsp?opcion=<%=opcion%>','');" >
                    </td></tr>
            </table>
