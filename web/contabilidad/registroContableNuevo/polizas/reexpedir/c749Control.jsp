<%-- 
    Document   : c749Control
    Created on : 22/02/2017, 05:58:01 PM
...
<%
 ...
 sbAutentifica.setPagina("c749Control");
...
try{
   ...
   cierres = new ArrayList();
     ...
     for(int z=0; z < polizasSeleccionadas.length; z++){      
        ...
        chequesCancelados.add(pbCheque.select_consecutivo_m(conexion, lsIdChequeConta));
 //       chequesCancelados.add(lsIdChequeConta);
        pbCheque.update_rf_tr_chequesm_estatus(conexion, lsIdChequeConta, "3"); // Se cambia el estatus del cheque a cancelado via el poliza_id de la poliza original
      }//for(int z=0; z < polizasSeleccionadas.length; z++)
   conexion.commit();  
   for (int i=0;i<chequesCancelados.size();i++) {   
%>
<p>Se canceló el cheque manual número <b><font color='#003399'> <%=chequesCancelados.get(i).toString()%></font></b> </p>
